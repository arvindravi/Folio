...
//    tableView.register(CardCell.self, forCellReuseIdentifier: CardCell.reuseIdentifier)
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    guard segue.identifier == K.Segues.RecipeDetailSegue else { return }
    guard let recipeItemViewModel = sender as? RecipeItemViewModel else { return }
    if let recipeDetailVC = segue.destination as? RecipeDetailViewController {
      recipeDetailVC.recipeTitle = recipeItemViewModel.title
      recipeDetailVC.ingredients = recipeItemViewModel.ingredients?.components(separatedBy: ",")
    }
  }
...
extension RecipeTableViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = tableView.dequeueReusableCell(withIdentifier: CardCell.reuseIdentifier) as? CardCell else { return UITableViewCell() }
...
extension RecipeTableViewController: UITableViewDelegate {
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    if let viewModel = recipeViewModel.viewModel(at: indexPath.row) {
      performSegue(withIdentifier: K.Segues.RecipeDetailSegue, sender: viewModel)
    }
  }
